...
- Root `package.json`'s `"version"` field (currently `1.1.0`) lags behind the actual latest git tag (`v1.2.0`) - the `v1.2.0` release commit did not bump it. Reconcile this the next time a release is cut (see `docs/workflows/GIT.md`'s release process).
