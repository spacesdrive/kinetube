...
## Killing a yt-dlp process (Pause, Cancel, and timeouts)

`backend/utils/processTree.js` exports `killProcessTree(proc)` and is the only correct way to stop a yt-dlp-spawned process - never call `proc.kill()` directly on one. yt-dlp's release binaries (Windows, and macOS/Linux too) are PyInstaller "onefile" builds: the process Node's `spawn()` gets a handle to is a launcher that unpacks itself and execs a second, separate process to do the real download, then just waits on it. A plain `proc.kill('SIGTERM')` only ends the launcher - the real worker is orphaned and keeps running (and keeps writing progress to the shared stdout pipe, which is also why the parent's `'close'` event doesn't fire until the orphan exits on its own). `killProcessTree()` reaches the whole tree: `taskkill /pid <pid> /t /f` on Windows, `process.kill(-pid, 'SIGTERM')` against a POSIX process group elsewhere (the target must be spawned with `detached: true` on non-Windows platforms for the group to exist). See `DECISIONS.md` ADR-021 for how this was diagnosed - Pause silently completing the download in the background instead of stopping it - and its full reasoning.

Every yt-dlp spawn site (`backend/routes/download.js`'s single-video download, and `backend/routes/instagram.js`'s single-video download, bulk download, and metadata-fetch-with-timeout) uses this helper. `whisperManager.js`'s and `instagram.js`'s Python-subprocess kills do not need it - whisper.cpp's `whisper-cli` is a plain compiled binary and the Python helpers are run directly (`python3 script.py`), neither has yt-dlp's launcher/worker split.
